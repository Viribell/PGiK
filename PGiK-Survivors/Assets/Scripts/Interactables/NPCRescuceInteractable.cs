...
public class NPCRescuceInteractable : Interactable {
    ...
    private void Awake() {
        if ( rescueData.defaultSprite != null ) {
            visual.GetComponent<SpriteRenderer>().sprite = rescueData.defaultSprite;
        }

        if ( rescueData.depletedSprite != null ) {
            depletedVisual.GetComponent<SpriteRenderer>().sprite = rescueData.depletedSprite;
        }
